...
known ids `0x50` and `0x60` as named variants in the low-level/timed action
stream. Other still-undecoded ids are exposed as `Action::OpaqueDroppedAction`.
...
- Summary-dropped actions that are not exposed by default. With
  `extended-actions`, ids `0x50`, `0x60`, and `0x7b` are decoded as named
  variants; ids `0x02`, `0x62`, `0x69`, `0x6a`, and `0x7a` are emitted as
  opaque payloads.
...
| `0x60` | 0x60 | 2880 | 2880 | 100.00% | drop: cheat/map trigger text | `TriggerChatCommand` with `extended-actions` |
...
independent methods, with the WC3 Reforged binary as a third anchor.
...
| `0x50` | 5 bytes | 15 samples, all len 5 | len5=478548 / len11=478541 | solid |
| `0x60` | 8 fixed + null-terminated string (variable) | 242 samples, spread 10..73 | n/a (variable) | solid — spread *proves* variable, not fixed |
| `0x62` | 12 bytes | 1,346 samples, all len 12 | len12=478548 / len25=476879 | strong |
| `0x69` | 16 bytes | 4 samples, all len 16 | len16=478548 / len17=478544 | good |
| `0x6a` | 16 bytes | none (never last) | len16=478548 (ties len33) | **weak** — see below |
...
the parser's lengths are correct, including the Reforged-era `0x7a` (20) and `0x7b`
(16) that public w3g specs (w3gjs, Pangaea) do not document. It does **not** confirm
the internal field *semantics* of the still-opaque ids (`0x62`, `0x69`, `0x6a`,
`0x7a`); only their lengths. `0x7b`'s internal layout
(`net_tag(8) + ability FourCC(4) + order FourCC(4)`) is additionally validated by its
exact size and the `extended-actions` decode.

**Caveat — `0x6a`.** Only 6 occurrences exist in the entire corpus and none is the
last action in its block, so neither method can fully isolate it. 16 is supported by
analogy with its sibling `0x69` and is not contradicted by the sweep, but it is the
one length that would benefit from a live dynamic trace to make airtight.
